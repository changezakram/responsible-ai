...
Evaluation metrics are where bias becomes invisible. A model reporting 95% aggregate accuracy can simultaneously perform at 70% for a specific demographic — and standard validation will not surface that gap. In financial services, a credit model that passes SR 11-7 model validation on aggregate performance metrics can still produce outcomes that fail fair lending standards at the group level. Without disaggregated evaluation across protected classes, these failures remain hidden until they appear in production.
