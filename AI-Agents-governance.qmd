...
> Two recent incidents illustrate what happens when that boundary isn't enforced. The Replit database deletion described later in this piece is one example — an agent's tool access outlived the task it was meant to serve (see A Practical Path Forward for how the industry has responded). In a separate case in February 2026, an autonomous crypto trading agent with wallet access and no transaction limits sent tens of millions of tokens to a stranger after misjudging its own balance, turning what should have been a small donation into the loss of a meaningful share of the token's entire supply. Neither agent behaved unpredictably in the abstract — each did exactly what its permissions allowed it to do.</p>
...
While the five governance dimensions define what institutions need to govern, they do not explain how governance should operate in practice. Agentic AI requires governance to move beyond policies and periodic review into the execution path itself. The governance control plane provides that operational layer, evaluating requests against institutional controls before autonomous actions reach enterprise systems.

![**Figure 2. Agentic AI Governance Control Plane.** The governance control plane embeds institutional controls between agentic systems and enterprise execution. Rather than relying on AI systems to interpret and enforce policy, the control plane evaluates requests against policy, identity and authority, runtime controls, observability, intervention, and audit before approved actions reach enterprise systems.](images/governance-control-plane.png){#fig-control-plane width=90%}
